...
1. 毎回の実行で **JustWatchの `newTitles` クエリから直近の新着候補を最大N件** 取得する
   ...
   **重要な制約（実機確認済み）**: `first`に150以上を指定すると
   JustWatch側が `page too large`（`TOO_BIG`）エラーで拒否する。
   100までは成功することを確認済みなので、`new_titles_fetch_count`/
   `init_fetch_count`は100が実質的な上限。これは裏を返すと
   **「1回の実行間隔（1時間）の間に100件を超える新着が1プロバイダに
   追加されると、はみ出した分は検知すらされずに取りこぼされる」**
   というリスクでもある（`notify_limit_per_run`超過分がキューに残るのとは違い、
   こちらは本当に「見えなくなる」）。運用上は現実的に十分な余裕がある想定だが、
   もし将来的に取りこぼしが疑われる場合は実行間隔を短くする方向で調整する
   （`new_titles_fetch_count`自体はこれ以上増やせない）。
