...
| `errors.json` | `{エラー署名: 最終通知日時}` クールダウン記録 | **30日** |
...
### 6.2 errors.jsonも定期的に整理する理由

エラー署名には具体的な例外メッセージ（`{exc}`の中身）を含むため、微妙に
原因が違うエラーが起きるたびに新しいキーとして増えていく。放置すると
`{provider}_active.json`と違って際限なく肥大化するため、`main.broadcast_errors`
は**エラーの有無に関わらず毎回**`state_manager.prune_error_log`で30日より
古い記録を落として保存する（`ERROR_COOLDOWN_HOURS`=6時間よりずっと長く
取っているので、クールダウン判定そのものには影響しない）。

「エラーが起きた時だけ`save_error_log`する」実装だと、一度発生したエラーが
二度と起きない限り整理結果が保存されず古い記録が残り続けてしまうため、
エラーが0件の実行でも必ず整理・保存するようにしている。
