...
- 同じ種別のエラーが**3回連続**（`ERROR_STREAK_THRESHOLD`、毎時実行なので約3時間）
  起きるまでは通知しない。Animephiliaへの接続はタイムアウトで単発に失敗することが
  あり、その多くは次の実行で自動復旧する。1回目から通知すると「次回実行時に
  再試行します」という勝手に直る内容の通知だけが鳴り続けてしまう
- 通知した後も、同一種別のエラーは**6時間に1回まで**に間引く（`state/errors.json`で管理）。
  毎時実行なので、素直に送ると1日24通×2チャンネル飛んでしまう
- 連続回数は「実行ごと」に数える。今回の実行で起きなかった種別は連続が途切れた
  ものとして記録ごと捨て、次に起きたときは1回目から数え直す
- 例外として、Discordに400で拒否されて**1件の通知を諦めた**場合は、連続回数・
  クールダウンに関係なく**即時に通知**する。その件は二度とキューに戻らず同じ
  エラーも再発しないため、3回連続を待つと永久に通知されず、通知が失われた
  ことに誰も気づけない（2026-09-23修正）

**連続回数の数え方**: エラー文言には具体的な例外メッセージ（`timed out` /
`HTTP 503` 等）が含まれ、同じ不調が続いていても実行ごとに文言が揺れる。署名を
そのままキーにすると連続と見なせないため、`state_manager.error_stream_key`で
最初の`": "`より後ろを落とし、残った数字を`N`に潰した「種別キー」で数える
（`[netflix] Animephiliaからの新着取得に失敗しました` のような枕詞部分）。
...
一時的なネットワーク不調で次の実行では正常に戻っていた（このように単発で復旧する
ケースが多いことが、上記の「3回連続まで通知しない」の根拠でもある）。誤診断を避けるため、
...
- 含まない場合（`_request_with_retry`がリトライを使い切って諦めた
  タイムアウト/5xx等）は「接続に失敗しました。ネットワーク不調の
  可能性が高いです」という文言にする（この通知が出る時点で既に3回連続
  失敗しているため、「通常は次回実行で自動復旧します」とは書かない）
- 上記2つの文言はAnimephiliaからの取得エラーが含まれるときだけ付ける。
  Discord側のエラー（Webhook失効・5xx等）だけのときに「Animephiliaへの接続に
  失敗しました」と出すと見当違いの場所を調べさせてしまうため、その場合は
  「実行ログを確認してください」とだけ添える（2026-09-23修正）
...
| `errors.json` | `{種別キー: {streak, last_seen, last_notified}}` 連続回数とクールダウン記録 | 連続が途切れるまで |
...
### 6.2 errors.jsonが肥大化しない理由

エラー署名には具体的な例外メッセージ（`{exc}`の中身）が含まれるため、微妙に
原因が違うエラーが起きるたびに新しいキーとして増えかねない。現行実装では
`main.broadcast_errors`が**エラーの有無に関わらず毎回**
`state_manager.record_error_run`の結果で丸ごと上書きし、**今回の実行で
起きなかった種別は捨てる**ため、ファイルに残るのは「いま連続中のエラー」だけに
なる（正常時は`{}`）。連続回数の数え方そのものが整理も兼ねているので、
保持期間による間引き（旧`prune_error_log`）は廃止した。

エラーが0件の実行でも必ず保存するのは、整理のためだけでなく**連続回数のリセット**
のためでもある。保存しないと「連続5回目」の記録が残り、しばらく正常だった後に
1回だけ失敗しただけで即通知になってしまう。
...
| エラーを都度送信 | 1原因で各チャンネルに4通×毎時 | 1通へ集約＋3回連続から通知＋6時間クールダウン（5.1節） |
